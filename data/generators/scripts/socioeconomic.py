...
for i in range(N):
    ...
    # Ingresos familiares (COP) - Mayor variabilidad
    mean_log = np.log(1_200_000) if des else np.log(2_000_000)
    ing = np.exp(rng.normal(mean_log, 0.75))
    ...
    # Horas de trabajo semanales - Mayor variabilidad
    if sit == "No trabaja":
        horas = 0
    elif sit == "Medio tiempo":
        horas = int(rng.normal(20, 8))
        horas = max(10, min(30, horas))
    else:
        horas = int(rng.normal(40, 12))
